...
public class Djp_pfsoln {
	...
	public static Object[] pfsoln(double baseMVA, Bus bus0, Gen gen0, Branch branch0,
		...
		Sbus = Ybus.viewSelection(gbus, null).zMult(V, null);
		Sbus.assign(cfunc.conj).assign(V.viewSelection(gbus), cfunc.mult);

		/* update Qg for all generators */
		gen.Qg.assign(0);					// zero out all Qg
		gen.Qg.viewSelection(on).assign(Sbus.copy().getImaginaryPart().assign(dfunc.mult(baseMVA)));	// inj Q
		...
		for (int k = 0; k < ref.length; k++) {
			refgen = IntFactory1D.dense.make( nonzero(ggbus.copy().assign(ifunc.equals(ref[k]))) );
			gen.Pg.set(on[refgen.get(0)], Sbus.getRealPart().get(refgen.get(0)) * baseMVA + bus.Pd.get(ref[k]));	// inj P + local Pd
			if (refgen.size() > 1) {	// more than one generator at this ref bus
