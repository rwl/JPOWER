...
public class Djp_dAbr_dV {
	...
	public static DoubleMatrix2D[] jp_dAbr_dV(DComplexMatrix2D dSf_dVa, DComplexMatrix2D dSf_dVm,
		...
		DoubleMatrix2D dAf_dPf = DoubleFactory2D.sparse.diagonal(Sf.getRealPart().assign(dfunc.mult(2)));
		DoubleMatrix2D dAf_dQf = DoubleFactory2D.sparse.diagonal(Sf.getImaginaryPart().assign(dfunc.mult(2)));
		DoubleMatrix2D dAt_dPt = DoubleFactory2D.sparse.diagonal(St.getRealPart().assign(dfunc.mult(2)));
		DoubleMatrix2D dAt_dQt = DoubleFactory2D.sparse.diagonal(St.getImaginaryPart().assign(dfunc.mult(2)));

		// Partial derivative of apparent power magnitude w.r.t voltage phase angle.
		DoubleMatrix2D dAf_dVa = dAf_dPf.zMult(dSf_dVa.getRealPart(), null);
		dAf_dVa.assign(dAf_dQf.zMult(dSf_dVa.getImaginaryPart(), null), dfunc.plus);
		DoubleMatrix2D dAt_dVa = dAt_dPt.zMult(dSt_dVa.getRealPart(), null);
		...
		DoubleMatrix2D dAf_dVm = dAf_dPf.zMult(dSf_dVm.getRealPart(), null);
		dAf_dVm.assign(dAf_dQf.zMult(dSf_dVm.getImaginaryPart(), null), dfunc.plus);
		DoubleMatrix2D dAt_dVm = dAt_dPt.zMult(dSt_dVm.getRealPart(), null);
